...
public class LikedRecordServiceRedisImpl extends ServiceImpl<LikedRecordMapper, LikedRecord> implements ILikedRecordService {
    ...
    public Set<Long> isBizLiked(List<Long> bizIds) {
        ...
        // 2.查询点赞状态
        // 这里使用Redis的管道技术，批量执行SISMEMBER命令，减少网络开销
        // 返回值是一个List，里面的每个元素对应一个bizId的点赞状态，true表示已点赞，false表示未点赞
        ...
        // 3.返回结果
        // 这里使用IntStream.range遍历索引，过滤出已点赞的bizId，并收集到一个Set中返回
        return IntStream.range(0, objects.size())
                .filter(i -> (boolean) objects.get(i))// 过滤出为true的索引
                .mapToObj(bizIds::get)
